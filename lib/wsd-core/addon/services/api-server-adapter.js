...
export default Ember.Service.extend({
    ...
    headers: Ember.computed('session.refererProduct', function() {
        switch (this.get('configTarget')) {
            case 'WEB_SD':
                var productName = this.get('session.refererProduct');
                var xsrfHeaderName = `X-XSRF-TOKEN-${productName}`;
                var xsrfCookieName = `XSRF-TOKEN-${productName}`;
                return {
                    'X-Sysdig-Product': productName,
                    xsrfHeaderName: Cookie.get(xsrfCookieName),
                };
            default:
                return {};
        }
    ...
    init(...args) {
        ...
        if (this.get('configTarget') === 'WEB_SD') {
            ...
            Ember.$.ajaxSetup({
                ...
                beforeSend: (xhr) => {
                    var productName = this.get('headers.X-Sysdig-Product');
                    var xsrfHeaderName = `X-XSRF-TOKEN-${productName}`;
                    var xsrfHeaderValue = this.get('headers.' + xsrfHeaderName);
                    xhr.setRequestHeader('X-Sysdig-Product', productName);
                    xhr.setRequestHeader(xsrfHeaderName, xsrfHeaderValue);
                },
            });
        }
    },
