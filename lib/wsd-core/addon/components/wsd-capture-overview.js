...
export default Component.extend({
    ...
    drilldownInfo: computed('drilldownInfoParam', function() {
    ...
    dataStore: computed('filePath', 'sampleCount', 'timeWindow', 'filter', function() {
    ...
    timelinesData: computed('timelines', 'dataStore.metrics', function() {
        const timelines = this.get('timelines');
        const metrics = this.get('dataStore.metrics');

        if (isEmpty(metrics) === false && isEmpty(timelines) === false) {
    ...
    metricsData: computed('captureTimelines.timelines', 'dataStore.metrics', 'selectedMetric', function() {
    ...
    categories: computed('dataStore.info.categories', 'metricsData', function() {
        const categoryConfigurations = this.get('dataStore.info.categories');
        const metricsData = this.get('metricsData');

        if (isEmpty(categoryConfigurations) === false && isEmpty(metricsData) === false) {
    ...
    didInsertElement() {
        ...
        this.get('shortcutsService').bind(
            ...
            () => {
                if (isNone(this.get('selectedMetric')) === false) {
