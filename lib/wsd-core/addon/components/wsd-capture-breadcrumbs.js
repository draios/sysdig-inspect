...
export default Component.extend({
    ...
    breadcrumbItems: computed('viewsManager.store.views', 'steps', function() {
        ...
        if (isEmpty(steps)) {
            ...
            return steps.map((step, i, list) => {
                return new BreadcrumbItem(
                    step,
                    i > 0 ? list[i - 1] : null,
                    isEmpty(views) ? null : this.get('viewsManager').peekViewConfiguration(step.viewId),
                    i === list.length - 1,
                    i === 0
                );
            });
    ...
    isRoot: equal('breadcrumbItems.length', 1),
