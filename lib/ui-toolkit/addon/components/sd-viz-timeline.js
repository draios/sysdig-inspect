...
export default Ember.Component.extend({
    ...
    hoverMarkerStyle: Ember.computed('xScale', 'hoverTimestamp', function() {
        const hoverTimestamp = this.get('hoverTimestamp');

        if (Ember.isNone(hoverTimestamp) === false) {
            const xScale = this.get('xScale');
            return Ember.String.htmlSafe(`width: ${xScale.bandwidth()}px; left: ${xScale(hoverTimestamp) + DRAGGABLE_HANDLE_WIDTH}px;`);
        } else {
            return null;
        }
    }).readOnly(),
    ...
    leftAreaStyle: Ember.computed('leftAreaSize', function() {
        return Ember.String.htmlSafe(`width: ${this.get('leftAreaSize') + DRAGGABLE_HANDLE_WIDTH}px;`);
    }).readOnly(),
    ...
    rightAreaStyle: Ember.computed('rightAreaSize', function() {
        return Ember.String.htmlSafe(`width: ${this.get('rightAreaSize') + DRAGGABLE_HANDLE_WIDTH}px;`);
    }).readOnly(),
