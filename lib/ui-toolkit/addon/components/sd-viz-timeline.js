...
export default Ember.Component.extend({
    ...
    xScaleHover: Ember.computed('timeline', 'timelinesWidth', function() {
        if (this.get('timeline')) {
            return d3.scaleBand()
                .domain(this.get('timeline'))
                .range([0, this.get('timelinesWidth')])
    ...
    hoverMarkerStyle: Ember.computed('xScaleHover', 'hoverTimestamp', function() {
        const hoverTimestamp = this.get('hoverTimestamp');

        if (Ember.isNone(hoverTimestamp) === false) {
            const xScale = this.get('xScaleHover');
