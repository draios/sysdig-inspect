...
export default Component.extend({
    ...
    rowsConfiguration: computed('rows', 'selection', function() {
    ...
    columnsWidth: computed('columns', function() {
        const columns = this.get('columns');

        if (isNone(columns) === false) {
    ...
    tableWrapStyle: computed('width', 'columnsWidth', function() {
        const width = this.get('width');
        const columnsWidth = this.get('columnsWidth');

        if (isNone(width) === false && isNone(columnsWidth) === false) {
            if (width < columnsWidth) {
                return htmlSafe(`width: ${columnsWidth}px;`);
            } else {
    ...
    tableStyle: computed('width', 'columnsWidth', 'columns', function() {
        const width = this.get('width');
        const columns = this.get('columns');

        if (isNone(width) === false && isNone(columns) === false) {
            const columnsWidth = this.get('columnsWidth');
            const idealColumnWidth = this.get('idealColumnWidth');
            const columnWidthUnit = this.get('columnWidthUnit');
    ...
    scrollToSelection() {
        ...
        if (isEmpty(rows) || isEmpty(selection)) {
            return;
        }
    ...
    resetSize() {
        ...
        next(this, this.scrollToSelection);
    },
